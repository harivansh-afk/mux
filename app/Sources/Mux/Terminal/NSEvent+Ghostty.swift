...
extension NSEvent {
    ...
    ///
    /// This will not set the "text" or "composing" fields since these
    /// can't safely be set with the information or lifetimes given.
    ///
    /// The translationMods should be set to the modifiers used for actual
    /// character translation if available.
    func ghosttyKeyEvent(
        ...
        // a cString, we can't use self.characters because of garbage
        // collection. We have to let the caller handle this.
        key_ev.text = nil
        key_ev.composing = false

        // macOS provides no easy way to determine the consumed modifiers for
        // producing text. We apply a simple heuristic here that has worked
        // for years so far: control and command never contribute to the
        // translation of text, assume everything else did.
        key_ev.mods = PaneView.ghosttyMods(modifierFlags)
        key_ev.consumed_mods = PaneView.ghosttyMods(
    ...
    ///
    /// This namely contains logic to avoid control characters, since we
    /// handle control character mapping manually within Ghostty.
    var ghosttyCharacters: String? {
           ...
           let scalar = characters.unicodeScalars.first
        {
            ...
            // If we have a single value in the PUA, then it's a function key
            // and we don't want to send PUA ranges down to Ghostty.
