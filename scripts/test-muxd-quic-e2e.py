...
both classified by `mux-attach probe`.
"""
import json
import os
import shutil
...
def probe(mux_attach_bin, env, alias):
    return json.loads(run([mux_attach_bin, "probe", alias], env, check=False).stdout)
...
def main():
    ...
    try:
        ...
        with Daemon(
            ...
            client.kill()
            client.close()
            if probe(mux_attach_bin, env_local, "testbox").get("class") != "pin-mismatch":
            ...
            client.kill()
            client.close()
            if probe(mux_attach_bin, env_local, "testbox").get("class") != "token-rejected":
